        ...
        <section class="rounded-lg bg-white p-6 shadow-sm ring-1 ring-[#0A2A6B]/10 sm:p-8">
            ...
            <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="mt-7">
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    ...
                    <label class="grid gap-2 text-sm font-bold text-[#0A2A6B]">
                        Matric Number
                        <input name="matno" value="{{ old('matno', $user->matno) }}" required pattern="(HBAF|NBAF)/(2[1-5][A-Za-z]?)/[0-9]{4}" class="rounded-lg border border-[#0A2A6B]/15 px-4 py-3 font-normal uppercase text-[#2E2E2E] outline-none transition focus:border-[#F5B400] focus:ring-4 focus:ring-[#F5B400]/20">
